export const GIANT_ICON_SIZE = 100;
export const LARGE_ICON_SIZE = 30;
export const NORMAL_ICON_SIZE = 18;
export const SMALL_ICON_SIZE = 14;

export const LARGER_FONT_SIZE = 30;
export const LARGE_FONT_SIZE = 24;
export const NORMAL_FONT_SIZE = 16;
export const SMALL_FONT_SIZE = 12;

export const DEFAULT_FONT = 'Roboto';
export const DEFAULT_WEIGHT = 'Regular';
export const SECONDARY_FONT = 'sans-serif';

export const PREVIEW_FRAME_WIDTH = 88;
export const TIMELINE_OFFSET_X = 40;

export const BASE_TIMELINE_ELEMENT_DURATION_MS = 4000;

export const DEFAULT_PREVIEW_WIDTH = 1920;
export const DEFAULT_PREVIEW_HEIGHT = 1080;
export const DEFAULT_PREVIEW_SCALE = 2.3;

export const DEFAULT_SCALED_PREVIEW_WIDTH = (
  DEFAULT_PREVIEW_WIDTH / DEFAULT_PREVIEW_SCALE
);

export const DEFAULT_SCALED_PREVIEW_HEIGHT = (
  DEFAULT_PREVIEW_HEIGHT / DEFAULT_PREVIEW_SCALE
);

export const MIN_MEDIA_PANEL_WIDTH = 176;
export const DEFAULT_MEDIA_PANEL_WIDTH = 336;
export const MAX_MEDIA_PANEL_WIDTH = 496;

export const DEFAULT_SETTINGS_PANEL_WIDTH = 300;
